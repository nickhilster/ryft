...
/**
 * Non-streaming one-shot generation for lightweight tasks like
 * personalising example prompts. Returns the full response string,
 * or null if Browser AI isn't available / the call fails.
 */
export async function quickGenerateWithBrowserAi(
  model: string,
  systemPrompt: string,
  userPrompt: string,
): Promise<string | null> {
  if (!isBrowserAiSupported()) return null;
  try {
    const activeEngine = await ensureBrowserAiEngine(model);
